...
func TestStarPointInitialScore(t *testing.T) {
	g := newGame(StarPoint)
	a, b, done := g.GetScore()
	if a != "0" || b != "0" || done {
...
func TestStarPointTeamAWins_40_0(t *testing.T) {
	g := newGame(StarPoint)
...
func TestStarPointTeamAWins_40_15(t *testing.T) {
	g := newGame(StarPoint)
	playSequence(t, &g, []scoreStep{
		{"B", "0", "15", false},
		{"A", "15", "15", false},
...
func TestStarPointTeamAWins_40_30(t *testing.T) {
	g := newGame(StarPoint)
	playSequence(t, &g, []scoreStep{
		{"B", "0", "15", false},
		{"B", "0", "30", false},
...
func TestStarPointTeamBWins_0_40(t *testing.T) {
	g := newGame(StarPoint)
	playSequence(t, &g, []scoreStep{
		{"B", "0", "15", false},
		{"B", "0", "30", false},
...
func TestStarPointTeamBWins_15_40(t *testing.T) {
	g := newGame(StarPoint)
	playSequence(t, &g, []scoreStep{
		{"A", "15", "0", false},
		{"B", "15", "15", false},
...
func TestStarPointTeamBWins_30_40(t *testing.T) {
	g := newGame(StarPoint)
	playSequence(t, &g, []scoreStep{
		{"A", "15", "0", false},
		{"A", "30", "0", false},
...
func TestStarPointDeuce1_AAdvantage_AWins(t *testing.T) {
	g := newGame(StarPoint)
	playSequence(t, &g, []scoreStep{
		{"A", "15", "0", false},
		{"A", "30", "0", false},
...
func TestStarPointDeuce1_BAdvantage_BWins(t *testing.T) {
	g := newGame(StarPoint)
	playSequence(t, &g, []scoreStep{
		{"A", "15", "0", false},
		{"A", "30", "0", false},
...
func TestStarPointDeuce1_Via30_40(t *testing.T) {
	g := newGame(StarPoint)
	playSequence(t, &g, []scoreStep{
		{"B", "0", "15", false},
		{"B", "0", "30", false},
...
func TestStarPointDeuce2_AWins(t *testing.T) {
	g := newGame(StarPoint)
	playSequence(t, &g, []scoreStep{
		{"A", "15", "0", false},
		{"A", "30", "0", false},
...
func TestStarPointDeuce2_BWins(t *testing.T) {
	g := newGame(StarPoint)
	playSequence(t, &g, []scoreStep{
		{"A", "15", "0", false},
		{"A", "30", "0", false},
...
// SP → A wins
func TestStarPointStarPoint_AWins(t *testing.T) {
	g := newGame(StarPoint)
	playSequence(t, &g, []scoreStep{
		// reach D1
		{"A", "15", "0", false},
...
// SP → B wins
func TestStarPointStarPoint_BWins(t *testing.T) {
	g := newGame(StarPoint)
	playSequence(t, &g, []scoreStep{
		// reach D1
		{"A", "15", "0", false},
...
func TestStarPointScoreFrozenAfterWin(t *testing.T) {
	g := newGame(StarPoint)
	// A wins 40-0
	for range 4 {
		g.ScoreForA()
...
func TestStarPointGetScore_MatchesIsComplete(t *testing.T) {
	g := newGame(StarPoint)
...
func TestStarPointReverseScoreForA_SingleStep(t *testing.T) {
	g := newGame(StarPoint)
...
func TestStarPointReverseScoreForB_SingleStep(t *testing.T) {
	g := newGame(StarPoint)
	g.ScoreForB()
...
func TestStarPointReversePoint_MidGame(t *testing.T) {
	g := newGame(StarPoint)
	g.ScoreForA() // 15/0
	g.ScoreForA() // 30/0
	g.ScoreForA() // 40/0
...
func TestStarPointReversePoint_FullUnwind(t *testing.T) {
	g := newGame(StarPoint)
...
func TestStarPointReversePoint_AtRoot_IsNoOp(t *testing.T) {
	g := newGame(StarPoint)
...
func TestStarPointReversePoint_FromWinNode(t *testing.T) {
	g := newGame(StarPoint)
	g.ScoreForA() // 15/0
	g.ScoreForA() // 30/0
	g.ScoreForA() // 40/0
...
func TestStarPointReversePoint_ThroughDeuce1(t *testing.T) {
	g := newGame(StarPoint)
	// Advance to D1
	g.ScoreForA()
	g.ScoreForA()
...
func TestStarPointReversePoint_ThroughStarPoint(t *testing.T) {
	g := newGame(StarPoint)
	...
	a, b, _ := g.GetScore()
	if a != "40" || b != "SP*" {
		t.Fatalf("expected 40/SP*, got %s/%s", a, b)
	}

	g.ReverseScoreForB()
	a, b, _ = g.GetScore()
	if a != "SP" || b != "SP" {
		t.Errorf("rollback from 40/SP*: got %s/%s, want SP/SP", a, b)
	}
}

func TestStarPointReversePoint_ThroughStarPointFromLowerSide(t *testing.T) {
	g := newGame(StarPoint)
	// Reach SP/SP: 40/0 → 40/15 → 40/30 → D1 → A1/40 → D2 → A2/40 → SP/SP
	g.ScoreForA()
	g.ScoreForA()
	...
	a, b, _ = g.GetScore()
	if a != "SP" || b != "SP" {
		t.Errorf("rollback from SP/SP*: got %s/%s, want SP/SP", a, b)
	}

	g.ReverseScoreForA()
	a, b, _ = g.GetScore()
	if a != "40" || b != "A2" {
		t.Errorf("rollback from SP/SP: got %s/%s, want 40/A2", a, b)
	}
	...
	a, b, _ = g.GetScore()
	if a != "40" || b != "A2" {
		t.Errorf("rollback from 40/A2*: got %s/%s, want 40/A2", a, b)
	}

	g.ReverseScoreForA()
	a, b, _ = g.GetScore()
	if a != "D2" || b != "D2" {
		t.Errorf("rollback from 40/A2: got %s/%s, want D2/D2", a, b)
	}

	g.ReverseScoreForA()
	a, b, _ = g.GetScore()
	if a != "40" || b != "A1" {
		t.Errorf("rollback from D2/D2: got %s/%s, want 40/A1", a, b)
	}
	...
	a, b, _ = g.GetScore()
	if a != "40" || b != "A1" {
		t.Errorf("rollback from 40/A1*: got %s/%s, want 40/A1", a, b)
	}

	g.ReverseScoreForA()
	a, b, _ = g.GetScore()
	if a != "D1" || b != "D1" {
		t.Errorf("rollback from 40/A1: got %s/%s, want D1/D1", a, b)
	}
}

// Interleaving forward and reverse moves
func TestStarPointReversePoint_Interleaved(t *testing.T) {
	g := newGame(StarPoint)
	g.ScoreForA()        // 15/0
	g.ScoreForA()        // 30/0
...
func TestStarPointAllStraightWinPaths(t *testing.T) {
	type winPath struct {
		name   string
