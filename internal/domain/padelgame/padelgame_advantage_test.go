...
func TestAdvantageInitialScore(t *testing.T) {
	g := newGame(Advantage)
	a, b, done := g.GetScore()
	if a != "0" || b != "0" || done {
...
func TestAdvantageTeamAWins_40_0(t *testing.T) {
	g := newGame(Advantage)
...
func TestAdvantageTeamAWins_40_15(t *testing.T) {
	g := newGame(Advantage)
	playSequence(t, &g, []scoreStep{
		{"B", "0", "15", false},
		{"A", "15", "15", false},
...
func TestAdvantageTeamAWins_40_30(t *testing.T) {
	g := newGame(Advantage)
	playSequence(t, &g, []scoreStep{
		{"B", "0", "15", false},
		{"B", "0", "30", false},
...
func TestAdvantageTeamBWins_0_40(t *testing.T) {
	g := newGame(Advantage)
	playSequence(t, &g, []scoreStep{
		{"B", "0", "15", false},
		{"B", "0", "30", false},
...
func TestAdvantageTeamBWins_15_40(t *testing.T) {
	g := newGame(Advantage)
	playSequence(t, &g, []scoreStep{
		{"A", "15", "0", false},
		{"B", "15", "15", false},
...
func TestAdvantageTeamBWins_30_40(t *testing.T) {
	g := newGame(Advantage)
	playSequence(t, &g, []scoreStep{
		{"A", "15", "0", false},
		{"A", "30", "0", false},
...
func TestAdvantageDeuce1_AAdvantage_AWins(t *testing.T) {
	g := newGame(Advantage)
	playSequence(t, &g, []scoreStep{
		{"A", "15", "0", false},
		{"A", "30", "0", false},
...
func TestAdvantageDeuce1_BAdvantage_BWins(t *testing.T) {
	g := newGame(Advantage)
	playSequence(t, &g, []scoreStep{
		{"A", "15", "0", false},
		{"A", "30", "0", false},
...
// 40-30 → B scores → D1/D1 → A scores → Advantage A (A1/40) → A wins
func TestAdvantageReiterate_DeuceAdvantage_AWins(t *testing.T) {
	g := newGame(Advantage)
	playSequence(t, &g, []scoreStep{
		{"A", "15", "0", false},
		{"A", "30", "0", false},
...
// 40-30 → B scores → D1/D1 → B scores → Advantage B (40/A1) → B wins
func TestAdvantageReiterate_DeuceAdvantage_BWins(t *testing.T) {
	g := newGame(Advantage)
	playSequence(t, &g, []scoreStep{
		{"B", "0", "15", false},
		{"B", "0", "30", false},
...
func TestAdvantageDeuce1_Via30_40(t *testing.T) {
	g := newGame(Advantage)
	playSequence(t, &g, []scoreStep{
		{"B", "0", "15", false},
		{"B", "0", "30", false},
...
func TestAdvantageScoreFrozenAfterWin(t *testing.T) {
	g := newGame(Advantage)
	// A wins 40-0
	for range 4 {
		g.ScoreForA()
...
func TestAdvantageGetScore_MatchesIsComplete(t *testing.T) {
	g := newGame(Advantage)
...
func TestAdvantageReverseScoreForA_SingleStep(t *testing.T) {
	g := newGame(Advantage)
...
func TestAdvantageReverseScoreForB_SingleStep(t *testing.T) {
	g := newGame(Advantage)
	g.ScoreForB()
...
func TestAdvantageReversePoint_MidGame(t *testing.T) {
	g := newGame(Advantage)
	g.ScoreForA() // 15/0
	g.ScoreForA() // 30/0
	g.ScoreForA() // 40/0
...
func TestAdvantageReversePoint_FullUnwind(t *testing.T) {
	g := newGame(Advantage)
...
func TestAdvantageReversePoint_AtRoot_IsNoOp(t *testing.T) {
	g := newGame(Advantage)
...
func TestAdvantageReversePoint_FromWinNode(t *testing.T) {
	g := newGame(Advantage)
	g.ScoreForA() // 15/0
	g.ScoreForA() // 30/0
	g.ScoreForA() // 40/0
...
func TestAdvantageReversePoint_ThroughDeuce1(t *testing.T) {
	g := newGame(Advantage)
	// Advance to D1
	g.ScoreForA()
	g.ScoreForA()
...
func TestAdvantageReversePoint_Interleaved(t *testing.T) {
	g := newGame(Advantage)
	g.ScoreForA()        // 15/0
	g.ScoreForA()        // 30/0
...
func TestAdvantageAllStraightWinPaths(t *testing.T) {
	type winPath struct {
		name   string
